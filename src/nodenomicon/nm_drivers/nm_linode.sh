...
function read_node_data
{
	...
	API_URL=$( get_param $NODE_DATA_FILE api_url 'https://api.linode.com/v4' )
...
function create_node
{
	...
	API_URL=$( get_param $provider_cfg api-url 'https://api.linode.com/v4' )
	API_USER_AGENT=$( get_param $provider_cfg user-agent "$APP_BANNER" )
	NODE_TYPE=$( get_param $provider_cfg type 'g6-nanode-1' )
	NODE_IMAGE=$( get_param $provider_cfg image 'linode/debian10' )
	MAX_NODE_COUNT=$( get_param $provider_cfg max-node-count '98' )
	...
	# Define local vars
	local node_region=$( get_param $provider_cfg region 'us-central,us-east,us-west' )
	local api_endpoint="$API_URL/linode/instances"
	...
	if [ "$node_available_slots" -gt "0" ] ; then
		...
		http_result=$( $CURL_CMD --silent --request POST --header "Authorization: Bearer $API_KEY" --header 'Content-Type: application/json' --user-agent "$API_USER_AGENT" --data '{"type":"'"$NODE_TYPE"'","region": "'"$node_selected_region"'","image":"'"$NODE_IMAGE"'","authorized_keys":["'"$node_ssh_key"'"],"tags":["App-'"$NODE_GROUP_TAG"'"],"root_pass":"'"$node_ssh_passwd"'","label":"'"$NODE_LABEL"'"}' --output $api_output --dump-header $api_output_header --write-out '%{http_code}' "$api_endpoint"  )
	else
	...
	NODE_ID=$( cat $api_output | jq -r '.id' )
	NODE_IPV4=$( cat $api_output | jq -r '.ipv4[0]' )
	...
	while [ "$node_status" != "running" ] && [ "$try_count" -le "$CREATE_MAX_TRIES" ] ; do
		let "try_count+=1"
...
{
	local api_endpoint="$API_URL/linode/instances/$NODE_ID"
	...
	while [ "$( echo $http_result | grep -P '^2[0-9]{2}$' )" == "" ] && [ "$try_count" -le "$DELETE_MAX_TRIES" ] ; do
		...
		http_result=$( $CURL_CMD --silent --request DELETE --header "Authorization: Bearer $API_KEY" --user-agent "$API_USER_AGENT" --output $api_output --dump-header $api_output_header --write-out '%{http_code}' "$api_endpoint" )
...
function get_node_status
{
	...
	http_result=$( $CURL_CMD --silent --request GET --header "Authorization: Bearer $API_KEY" --user-agent "$API_USER_AGENT" --output $api_output --dump-header $api_output_header --write-out '%{http_code}' "$api_endpoint" )
	
	if [ "$( echo $http_result | grep -P '^2[0-9]{2}$' )" != "" ] ; then 
		cat $api_output | jq -r '.status'	
	else
		cp $api_output $api_output_header $ERROR_DIR
...
function get_region
{
	...
	if [ "$node_region" == "auto" ] ; then
		# Get all available regions that supports Linodes
		http_result=$( $CURL_CMD --silent --request GET --user-agent "$API_USER_AGENT" --output $api_output --dump-header $api_output_header --write-out '%{http_code}' "$api_endpoint" )

		if [ "$( echo $http_result | grep -P '^2[0-9]{2}$' )" != "" ] ; then 
			cat $api_output | jq -c '.data[] | {id, status, capabilities}' | grep -P '"status":"ok"' | grep -P 'Linodes' | jq -r .id | shuf | head -n 1 
		else
			cp $api_output $api_output_header $ERROR_DIR
			echo "api_error_$http_result" 
		fi
...
function get_node_count
{
	...
	# get data
	local api_endpoint="$API_URL/linode/instances?page_size=25"
 	local api_output=$( gen_api_output )
 	local api_output_header="${api_output}.header"

	# Get all linodes, account wide. Get the first page (to reduce result size), then get the 'results' data from pagination (the node count).
	local http_result=$( $CURL_CMD --silent --request GET --header "Authorization: Bearer $API_KEY" --user-agent "$API_USER_AGENT" --output $api_output --dump-header $api_output_header --write-out '%{http_code}' "$api_endpoint" )

	if [ "$( echo $http_result | grep -P '^2[0-9]{2}$' )" != "" ] ; then 
		cat $api_output | jq -c '.results'
	else
		cp $api_output $api_output_header $ERROR_DIR
		echo "api_error_$http_result" 
	fi
